...
const configYamlPath = path.join(__dirname, "../.movement/config.yaml");
...
function initCustomNetwork(restUrl, faucetUrl) {
  let command = `[ ! -f .movement/config.yaml ] || rm .movement/config.yaml; echo '' | aptos init --network custom --rest-url ${restUrl}`;
  if (faucetUrl) {
  ...
  exec(command, (error, stdout, stderr) => {
    if (error) {
      console.log(stdout);
      console.log(
        `Failed to initialize custom network. Please check your network settings.`
      );
      process.exit(1);
    }
...
function initPredefinedNetwork(network) {
  ...
  const command = `[ ! -f .movement/config.yaml ] || rm .movement/config.yaml; echo '' | aptos init --network ${network}${skipFaucet}`;
...
function updateMoveTomlAfterInit() {
  try {
    ...
    const newAddress = `0x${config.profiles.default.account.replace(
      /^0x/,
      ""
    )}`; // Ensure 0x prefix
...
if (network in networks) {
  const { "rest-url": configRestUrl, "faucet-url": configFaucetUrl } =
    networks[network];
